-- 008_sync_badges.sql
-- Badge sync gap: the CLI computes earned badges locally (src/badges/check.js)
-- but never sent them to the cloud, so the dashboard's Badges tab (which reads
-- the `badges` table via get-dashboard) showed 0 despite locally-earned badges.
--
-- Fix: extend the atomic sync RPC to also accept the user's earned badges and
-- upsert them in the SAME transaction as sessions/turns/daily. Idempotent:
-- ON CONFLICT (user_id, badge_type) DO NOTHING, so earned_at is written once and
-- never rewritten on re-sync.
--
-- Signature change: the prior RPC is sync_user_batch(text, jsonb). We DROP it and
-- recreate as sync_user_batch(text, jsonb, jsonb default '[]'). The default means
-- an older edge function calling with only {p_anonymous_id, p_sessions} still
-- resolves to this function (PostgREST fills the defaulted arg), so the migration
-- and the edge-function redeploy can land in either order.
--
-- Deploy ordering: AFTER 005/006/007. Additive + idempotent; safe to re-run.
-- PRIVACY: badge_type is a non-PII enum label + a timestamp — no prompts/code/
-- paths (Decision #5 unchanged).
...
create or replace function sync_user_batch(p_anonymous_id text, p_sessions jsonb, p_badges jsonb default '[]'::jsonb)
returns jsonb
language plpgsql
as $$
declare
...
begin
  ...
  for v_badge in select jsonb_array_elements(coalesce(p_badges, '[]'::jsonb)) loop
    ...
    v_badges_synced := v_badges_synced + 1;
  end loop;

  -- Daily summaries are derived purely from `turns`; only recompute when sessions
  -- were actually processed (a badges-only push leaves turns — and therefore the
  -- summaries — unchanged, so the delete+recompute would be a no-op).
  ...
  return jsonb_build_object('synced', v_session_count, 'turns_synced', v_turns_synced, 'badges_synced', v_badges_synced);
end;
$$;
...
revoke all on function sync_user_batch(text, jsonb, jsonb) from public, anon, authenticated;
grant execute on function sync_user_batch(text, jsonb, jsonb) to service_role;
